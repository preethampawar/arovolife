...
**Centre registry (2026-08-30).** The centres list is a filterable registry — status, state (fixed dropdown), city, type (company / distributor), phase, owner ADN and name — with the registry columns the client asked for: city, state, owner, type, phase, status, contact person and number, weekly off and a truncated address. Each centre carries a full premises record (address lines, landmark, city, pincode, state, property type, size in sq ft, self-declared distance to the nearest centre, operating hours, weekly off, contact details); the old free-text "location" and "district" values were copied into the address and city fields. From the list an admin can **Set default** (the centre pre-selected at registration Step 11), **Deactivate** (reason required; the centre leaves every picker but distributors already connected to it are not moved) and **Activate**. Changing the owner ADN on the Edit form transfers the centre. All of it is audit-logged.

**Applications (flag `compensation.arete_center_applications`).** When the flag is on, a distributor can apply to open a centre from **My Arete Centre** in the account menu: their identity block is read from the distributor record (no PAN, Aadhaar or bank details are collected), they enter the centre name and premises, upload ownership/rent proof, up to five premises photos and an address proof (trade licence and a photo are optional; PDF/JPG/PNG ≤ 5 MB, private storage), and accept five versioned declarations — including that the centre is for training and support only, never a retail store. The premises size field is pre-filled with, and validated against, the **Minimum premises size** setting (`adc.min_premises_sqft`, admin-owned, default 350 sq ft). The **ADC Applications** queue lists open applications first with status / state / search filters. Reviewers with `compliance.discipline` can **Mark under review**, **Approve** (creates the centre active at Phase 1 with the applicant as owner), **Request changes** (reason emailed; the applicant edits and resubmits) or **Reject** (reason emailed). Every decision is audit-logged and every document view is audit-logged. Applying is free and shows no income figures; the phase income ladder stays admin-only.
