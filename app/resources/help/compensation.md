...
The **ADC Monthly Calculation** report shows one **month header** per month on view — centres paid, their combined member BV, what the ADC rate would pay before caps, the monthly cap, the bonus actually paid and Computed — followed by the per-centre formula symbolically (member BV = net BV of the centre's active members; flat = ⌊member BV × rate⌋; bonus = min(flat, cap), a centre's own override may only lower the cap) and then with the month's combined values, noting how many centres were capped. Rate and cap are current plan settings; each row's bonus is what the run froze. The report and the engine share one arithmetic method, so the "before caps" figure can never drift from what the run pays.
