...
| Fulfilment · `shipments.gateway` | `manual` · `shiprocket` | `manual` | Who booked the courier leg. Only a `shiprocket` parcel gets a buyer tracking link and tracking updates. (Live since 2026-09-24.) |
| Fulfilment · `shipments.courier_status` | free text from the courier, e.g. `IN TRANSIT`, `DELIVERED`, `RTO INITIATED`, `LOST` | null | The courier's own last status, recorded only after our API check confirms it. `RTO…`, `LOST`, `DAMAGED`, `DESTROYED` and `CANCELED` put the order on the *Courier exceptions* list. An `RTO…` status also sets `shipments.status` to `returned_to_origin`. (Live since 2026-09-24.) |
