...
<div class="mb-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
    Only use this when Retry fails and a confirmed sale-based BV match was not credited due to a system error. You must reference the specific GSB cut-off result ID — this ties the credit to the product sale chain and makes it payable.
</div>
<form method="POST" action="{{ route('admin.compensation.manual-controls.credit') }}"
      data-confirm="This will credit a custom amount to the distributor's wallet tied to the referenced cut-off result."
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
            <label class="block text-xs font-medium text-gray-700 mb-1">
                GSB Cut-off Result ID
                <x-help-tip text="The numeric ID of the gsb_cutoff_results row for the date/slab that should have paid out. Find it in the GSB History tab of the distributor's compensation page." />
            </label>
            <input type="number" name="gsb_cutoff_result_id" min="1" required placeholder="e.g. 42"
                   class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-brand-400 focus:outline-none">
        </div>
    </div>
    <div class="mb-4">
