...
<div class="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-3 text-xs text-blue-800">
    ...
    The MSB is a percentage of the sponsee's daily GSB credited to their sponsor. Search by sponsor ADN or name.
</div>

{{-- Filters --}}
<form method="GET" class="flex flex-wrap items-center gap-3 mb-4">
    <input type="text" name="q" value="{{ $q ?? '' }}"
           placeholder="Search sponsor ADN or name…"
           class="rounded-lg border border-gray-300 px-3 py-1.5 text-sm w-56">
    ...
    @endif
    <a href="{{ route('admin.compensation.msb-calculation.export', array_filter(['q' => $q, 'from' => $from, 'to' => $to, 'status' => $status])) }}"
            ...
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-3 py-2 text-right text-gray-500 font-medium">
                        Sponsee GSB
                        <x-help-tip text="The sponsee's net GSB income that day, which the MSB rate is applied to." />
                    </th>
                    <th class="px-3 py-2 text-center text-gray-500 font-medium">Rate</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-3 py-2 text-right text-gray-800">
                        ₹{{ \Illuminate\Support\Number::format($row->sponsee_gsb_paise / 100, 2) }}
                    </td>
                    <td class="px-3 py-2 text-center">
                        <span class="inline-flex px-2 py-0.5 rounded font-semibold text-[11px] bg-purple-50 text-purple-700">
                            {{ $row->mb_rate_pct }}%
                        </span>
                    </td>
                    <td class="px-3 py-2 text-right">
                        <span class="font-semibold {{ $row->status === 'failed' ? 'text-red-600' : 'text-green-700' }}">
                            ₹{{ \Illuminate\Support\Number::format($row->mb_paise / 100, 2) }}
                        </span>
                        @if($row->mb_tds_paise > 0)
                        <span class="block text-[10px] text-gray-400 font-normal">
                            gross ₹{{ \Illuminate\Support\Number::format($row->mb_gross_paise / 100, 2) }} · TDS ₹{{ \Illuminate\Support\Number::format($row->mb_tds_paise / 100, 2) }}
                        </span>
                        @endif
                    </td>
                    <td class="px-3 py-2 text-center">
