...
<div class="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
    The earning week runs Wednesday to Tuesday and is paid the Tuesday one week after it closes: the batch dated Tuesday 18 August pays income earned from Wednesday 5 August to Tuesday 11 August. "Earnings through" is the last day each batch pays for; anything earned after it waits for the next Tuesday. The week is keyed on the day the income was earned — the GSB cut-off date, the mentorship cut-off day — not on when the credit reached the wallet. Each batch shows total gross, deductions (repurchase + admin charge + TDS), and net transferred. Minimum payout is ₹{{ $minPayout }} — below-minimum balances roll over to the next week. If the batch failed for one distributor there is no admin control that pays them on their own: a batch is built for the whole population or not at all, so retry the batch from <a href="{{ route('admin.compensation.engine-runs.index') }}" class="underline">Engine Runs</a>, and if it still misses them, tell the platform team with the distributor's ADN and the batch date.
</div>
@enddeveloper

<x-filter-bar :filters="$filters" :exports="[
    ['label' => 'Download Excel', 'url' => route('admin.compensation.weekly-payouts.export', $filters->toQuery() + ['format' => 'xlsx'])],
    ['label' => 'CSV', 'url' => route('admin.compensation.weekly-payouts.export', $filters->toQuery() + ['format' => 'csv'])],
]" />

<x-ui.card flush>
    @if($batches->isEmpty())
    <x-ui.empty-state title="No payout batches yet."
                      description="Weekly payout not yet active." />
    @else
