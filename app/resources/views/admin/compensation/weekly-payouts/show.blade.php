...
<div class="mb-4 flex items-center justify-between">
    ...
    <div class="flex items-center gap-3">
        ...
        <form method="POST" action="{{ route('admin.compensation.weekly-payouts.approve', $batch) }}"
              ...
              data-confirm="Approve this payout batch of ₹{{ \App\Modules\Shared\Support\IndianNumber::format($batch->total_net_paise / 100, 2) }} to {{ $batch->distributor_count }} distributor(s)?"
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        ...
        <p class="mt-1 text-lg font-bold text-gray-900">₹{{ \App\Modules\Shared\Support\IndianNumber::format($batch->total_gross_paise / 100, 2) }}</p>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        <p class="text-xs font-medium text-gray-500 uppercase tracking-wider flex items-center gap-1">
            Deductions <x-help-tip text="Repurchase deduction + admin charge + TDS across all line items." />
        </p>
        <p class="mt-1 text-lg font-bold text-red-600">₹{{ \App\Modules\Shared\Support\IndianNumber::format($batch->total_deductions_paise / 100, 2) }}</p>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">Net to transfer</p>
        <p class="mt-1 text-lg font-bold text-green-700">₹{{ \App\Modules\Shared\Support\IndianNumber::format($batch->total_net_paise / 100, 2) }}</p>
    </div>
</div>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                @php
                    $lc = [
                        ...
                        'no_bank_account' => 'bg-gray-100 text-gray-500',
                        'web_only'       => 'bg-gray-100 text-gray-500',
                    ];
                @endphp
                <tr>
                    ...
                    <td class="px-3 py-2 text-right font-semibold {{ $line->net_transferred_paise > 0 ? 'text-green-700' : 'text-gray-400' }}">
                        ₹{{ \App\Modules\Shared\Support\IndianNumber::format($line->net_transferred_paise / 100, 2) }}
                    </td>
