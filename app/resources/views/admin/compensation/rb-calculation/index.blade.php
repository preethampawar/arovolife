...
<div class="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-3 text-xs text-blue-800">
    Global monthly Rank Bonus (RB) calculation table — one row per distributor per month.
    Rank indicates the tier pool (1 = first rank pool, 2 = second, etc.). Search by ADN or name.
</div>
...
<form method="GET" class="flex flex-wrap items-center gap-3 mb-4">
    ...
    <input type="number" name="rank" value="{{ $rank ?? '' }}" min="1" placeholder="Rank #"
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
    @if($rows->isEmpty())
    <p class="px-6 py-8 text-sm text-gray-400 text-center">No Rank Bonus records found.</p>
    @else
        ...
        <table class="w-full text-xs">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-3 py-2 text-right text-gray-500 font-medium">Income (Net RB)</th>
