            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="{{ $row->status === 'failed' ? 'bg-red-50' : '' }}">
                    ...
                    <td class="px-3 py-2 text-right">
                        {{ $row->gross_gsb_paise ? '₹'.\Illuminate\Support\Number::format($row->gross_gsb_paise / 100, 2) : '—' }}
                    </td>
                    <td class="px-3 py-2 text-right text-gray-500">
                        {{ $row->admin_charge_paise ? '₹'.\Illuminate\Support\Number::format($row->admin_charge_paise / 100, 2) : '—' }}
                    </td>
                    <td class="px-3 py-2 text-right text-gray-500">
                        {{ $row->tds_paise ? '₹'.\Illuminate\Support\Number::format($row->tds_paise / 100, 2) : '—' }}
                    </td>
