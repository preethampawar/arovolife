...
<form method="GET" class="flex flex-wrap items-center gap-3 mb-4">
    ...
    @endif
    <a href="{{ route('admin.compensation.fb-calculation.export', array_merge(array_filter(['q' => $q, 'month' => $month, 'status' => $status]), ['format' => 'xlsx'])) }}"
       class="ml-auto px-3 py-1.5 rounded-lg border border-gray-300 bg-white text-xs text-gray-700 hover:bg-gray-50">
        ↓ Download Excel
    </a>
    <a href="{{ route('admin.compensation.fb-calculation.export', array_merge(array_filter(['q' => $q, 'month' => $month, 'status' => $status]), ['format' => 'csv'])) }}"
