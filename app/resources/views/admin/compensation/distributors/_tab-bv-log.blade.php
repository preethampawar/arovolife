    ...
    <table class="w-full text-xs">
        <thead class="bg-gray-50">
            <tr>
                ...
                <th class="px-3 py-2 text-right text-gray-600">BV <x-help-tip text="Positive = accrual (purchase). Negative = reversal (cancellation or refund)." /></th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-50">
            ...
            <tr>
                ...
                <td class="px-3 py-2 text-gray-600">
                    @if($row->order_id)
                    <a href="{{ route('admin.commerce.orders.show', $row->order_id) }}" class="text-brand-700 hover:underline font-mono">#{{ $row->order_id }}</a>
                    @else
                    —
                    @endif
                </td>
