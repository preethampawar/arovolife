...
<form method="GET" class="flex flex-wrap items-center gap-3 mb-4">
    ...
    @endif
    <a href="{{ route('admin.compensation.gsb-calculation.export', array_merge(array_filter(['q' => $q, 'from' => $from, 'to' => $to, 'status' => $status, 'slab' => $slab]), ['format' => 'xlsx'])) }}"
       class="ml-auto px-3 py-1.5 rounded-lg border border-gray-300 bg-white text-xs text-gray-700 hover:bg-gray-50">
        ↓ Download Excel
    </a>
    <a href="{{ route('admin.compensation.gsb-calculation.export', array_merge(array_filter(['q' => $q, 'from' => $from, 'to' => $to, 'status' => $status, 'slab' => $slab]), ['format' => 'csv'])) }}"
...
@foreach($dayPools as $pool)
    @include('admin.compensation._formulas.gsb-day', ['pool' => $pool, 'slab3Score' => $slab3Score, 'open' => count($dayPools) === 1])
@endforeach
@if($hiddenDays > 0)
