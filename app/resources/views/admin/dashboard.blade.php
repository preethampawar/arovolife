...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm hover:shadow-md transition-shadow">
        ...
        <p class="text-xs uppercase tracking-wider text-gray-500 font-medium mb-1">{{ $card['label'] }}</p>
        ...
        <div class="divide-y divide-gray-100">
            ...
            <div class="px-6 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors">
                <div>
                    ...
                    <p class="text-xs text-gray-500">{{ $d->full_name ?? $d->email }}</p>
                </div>
                <div class="text-right">
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">{{ \Carbon\Carbon::parse($d->effective_date)->format('d M Y') }}</p>
                </div>
            </div>
            @empty
            <p class="px-6 py-6 text-sm text-gray-500 text-center">No distributors yet.</p>
            @endforelse
        </div>
    </div>
        ...
        <div class="divide-y divide-gray-100">
            ...
            <div class="px-6 py-3 hover:bg-gray-50 transition-colors">
                ...
                <p class="text-xs text-gray-500 mt-0.5">
                    ...
                    @if($log->actor_email) · by {{ $log->actor_email }}@endif
                </p>
            </div>
            @empty
            <p class="px-6 py-6 text-sm text-gray-500 text-center">No audit events yet.</p>
            @endforelse
        </div>
    </div>
