...
<form method="POST" action="{{ route('admin.inventory.transfers.store') }}" class="max-w-5xl space-y-6">
    ...
    <x-ui.card padding="p-6 space-y-4 overflow-x-auto">
        ...
        <button type="button" id="transferAddLine" class="text-sm text-brand-700 hover:text-brand-800 font-medium">{{ svg('lucide-plus', 'w-3.5 h-3.5 inline-block align-[-2px]', ['aria-hidden' => 'true']) }} Add line</button>
    </div>

    </x-ui.card> class="flex items-center gap-3">
        <x-ui.button >Create transfer</x-ui.button>
