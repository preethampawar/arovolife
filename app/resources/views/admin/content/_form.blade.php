...
<div class="grid grid-cols-1 gap-5">
    ...
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1.5">Content Type <x-help-tip text="Categorise this page as a blog post, seminar, news article, hub page, or leave blank for general pages (e.g. policy, legal)." /></label>
        <select name="type"
            class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent">
            <option value="" {{ old('type', $page->type) === null || old('type', $page->type) === '' ? 'selected' : '' }}>(blank) — General</option>
            <option value="blog" {{ old('type', $page->type) === 'blog' ? 'selected' : '' }}>blog — Blog</option>
