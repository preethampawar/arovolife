...
<form method="GET" action="{{ route('admin.audit-log') }}" class="flex flex-wrap gap-3 mb-6">
    ...
    <select name="subject_type"
        class="rounded-lg bg-white border border-gray-200 px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-brand-500">
    ...
    <a href="{{ route('admin.audit-log') }}" class="px-4 py-2 rounded-lg bg-white border border-gray-200 text-sm text-gray-600 hover:text-white transition-colors">
        Clear
    </a>
    @endif
</form>
...
<div class="flex flex-wrap gap-2 mb-6">
    ...
    <a href="{{ route('admin.audit-log', ['action' => $grp]) }}"
       class="px-2.5 py-1 rounded-full text-xs border border-gray-200 bg-white text-gray-600 hover:border-brand-500 hover:text-brand-600 transition-colors">
        {{ $grp }} <span class="text-gray-400">({{ $cnt }})</span>
    </a>
    @endforeach
</div>
@endif
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-200 bg-gray-50/50">
                    <th class="text-left px-4 py-3 text-xs text-gray-500">Time</th>
                    <th class="text-left px-4 py-3 text-xs text-gray-500">Action</th>
                    <th class="text-left px-4 py-3 text-xs text-gray-500">Subject</th>
                    <th class="text-left px-4 py-3 text-xs text-gray-500">Actor</th>
                    <th class="text-left px-4 py-3 text-xs text-gray-500">Details</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-800">
                ...
                <tr class="hover:bg-white/40 transition-colors">
                    ...
                    <td class="px-4 py-3 text-xs text-gray-500">{{ $log->actor_email ?? 'system' }}</td>
                    <td class="px-4 py-3 text-xs">
                        @if($log->details)
                        <details>
                            <summary class="text-gray-400 cursor-pointer hover:text-gray-600">view</summary>
                            <pre class="mt-1 text-gray-500 bg-white rounded p-2 text-xs overflow-x-auto max-w-xs">{{ json_encode(json_decode($log->details), JSON_PRETTY_PRINT) }}</pre>
                        </details>
                        @else
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500">No audit entries found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
