...
<table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
    <tr>
        <td>
            ...
            @if(!empty($carrier) || !empty($awbNo))
            <p style="margin: 0 0 14px 0; font-size: 15px; line-height: 24px; color: #374151;">
                @if(!empty($carrier))Courier: <strong style="color: #111827;">{{ $carrier }}</strong>@endif
                @if(!empty($carrier) && !empty($awbNo))<br>@endif
                @if(!empty($awbNo))Tracking number: <span style="font-family: 'SFMono-Regular', Menlo, Consolas, monospace;">{{ $awbNo }}</span>@endif
            </p>
            @endif
            @if(!empty($trackingUrl))
            <p style="margin: 0 0 14px 0; font-size: 15px; line-height: 24px;">
                <a href="{{ $trackingUrl }}" style="color: #0a719f;">Track your parcel</a>
            </p>
            @endif
