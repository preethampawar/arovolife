...
<div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <div class="flex gap-1 mb-6 border-b border-gray-200">
        <a href="{{ route('orders.index') }}"
           class="px-4 py-2 text-sm font-medium border-b-2 border-transparent text-gray-600 hover:text-gray-700 hover:border-gray-300 -mb-px">
            My Purchases
        </a>
    ...
    @if($sales->isEmpty())
        <div class="bg-white rounded-2xl border border-gray-200 p-12 text-center">
            ...
            <p class="text-sm text-gray-600">Share a product link or Easy Purchase cart with a customer to get started.</p>
        </div>
    @else
        {{-- Desktop table (sm+) --}}
            ...
            <table class="w-full text-sm min-w-[680px]">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-gray-600">{{ $sales->firstItem() + $loop->index }}</td>
                        <td class="px-4 py-3">
                            <a href="{{ route('orders.sales.show', $order->order_no) }}" class="font-mono text-brand-700 hover:text-brand-800">{{ $order->order_no }}</a>
                        </td>
            ...
            <a href="{{ route('orders.sales.show', $order->order_no) }}"
               class="block bg-white rounded-2xl border border-gray-200 p-4 hover:border-brand-300 transition-colors">
                <div class="flex items-start justify-between gap-2 mb-1">
                    <span class="font-mono text-brand-700 font-medium text-sm">{{ $order->order_no }}</span>
                    @include('partials.order-status-badge', ['status' => $order->status])
                </div>
                <p class="text-xs text-gray-600 mb-2">{{ $order->customer?->display_name ?? 'Customer' }}</p>
                <div class="flex justify-between items-center text-sm">
                    <span class="text-gray-600">{{ $order->placed_at?->format('d M Y') }}</span>
