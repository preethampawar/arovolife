...
<div>
    ...
    @if($orders->isEmpty())
        <div class="bg-white rounded-2xl border border-gray-200 p-12 text-center">
            ...
            <a href="{{ route('shop.index') }}" class="text-brand-700 hover:text-brand-800 font-medium">Browse products →</a>
        </div>
    @else
        {{-- Desktop table (sm+) --}}
            ...
            <table class="w-full text-sm min-w-[560px]">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-gray-600">{{ $orders->firstItem() + $loop->index }}</td>
                        <td class="px-4 py-3">
                            <a href="{{ route('orders.show', $order->order_no) }}" class="font-mono text-brand-700 hover:text-brand-800">{{ $order->order_no }}</a>
                        </td>
