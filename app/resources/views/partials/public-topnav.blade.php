            ...
            <a href="{{ route('shop.cart') }}"
               ...
               aria-label="Cart{{ $cartItemCount > 0 ? ' — '.$cartItemCount.' item'.($cartItemCount === 1 ? '' : 's') : '' }}">
                <span class="relative inline-flex">
                    ...
                    @if($cartItemCount > 0)
                    <span class="absolute -top-2 -right-2.5 inline-flex items-center justify-center min-w-[17px] h-[17px] px-1 rounded-full bg-white text-brand-700 text-[10px] font-bold leading-none shadow ring-1 ring-brand-500/20">{{ $cartItemCount > 99 ? '99+' : $cartItemCount }}</span>
                    @endif
                </span>
            </a>
