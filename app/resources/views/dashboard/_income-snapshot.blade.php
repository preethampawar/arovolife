...
<div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        <div class="rounded-xl bg-gradient-to-br from-brand-600 to-brand-800 text-white p-4">
            ...
            <p class="text-2xl font-bold leading-tight">{{ $walletBalancePaise !== null ? $fmt::rupees($walletBalancePaise) : '₹—' }}</p>
        </div>
        <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
            <div class="flex items-center justify-between mb-1">
                <p class="text-[11px] uppercase tracking-wider font-semibold text-gray-600">Credited this month</p>
                <x-help-tip text="Total bonus credits that reached your wallet since the 1st of this month." />
            </div>
            <p class="text-2xl font-bold text-gray-900 leading-tight">{{ $fmt::rupees((int) $monthTotal) }}</p>
        </div>
        <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
            <div class="flex items-center justify-between mb-1">
                <p class="text-[11px] uppercase tracking-wider font-semibold text-gray-600">Next weekly payout</p>
                <x-help-tip text="Weekly bonuses transfer to your bank every Tuesday (IST){{ ($keyDates['hasMonthlyBonuses'] ?? false) ? '; monthly bonuses on the 9th' : '' }}, provided your balance meets the minimum payout." />
            </div>
            <p class="text-2xl font-bold text-gray-900 leading-tight">{{ $keyDates['nextWeeklyPayout']->format('D, d M') }}</p>
            @if($keyDates['hasMonthlyBonuses'] ?? false)
                <p class="text-[11px] text-gray-600 mt-1">Monthly: {{ $keyDates['nextMonthlyPayout']->format('d M') }}</p>
            @endif
        </div>
    </div>

    {{-- Per-bonus rows --}}
    <div class="divide-y divide-gray-100 border-y border-gray-100 mb-5">
        <div class="grid grid-cols-[1fr_auto_auto] gap-x-4 py-2 text-[11px] uppercase tracking-wider font-semibold text-gray-600">
        ...
        @foreach($bonusSummary as $row)
            <a href="{{ route($row['route']) }}" class="grid grid-cols-[1fr_auto_auto] gap-x-4 items-center py-2.5 text-sm hover:bg-gray-50 -mx-2 px-2 rounded-lg transition">
