@extends('layouts.app')
@section('title', 'My Income — Overview')
...
<div>
    ...
    <div class="grid grid-cols-1 {{ $gsbOn ? 'sm:grid-cols-3' : 'sm:grid-cols-1' }} gap-4 mb-6">
        ...
        @php
            ...
            // Pending-until-cut-off rule (client, 2026-08-29): the personal
            // purchase BV is not attached to a side on the page — it sits on the
            // info icon until the 23:59 cut-off credits it to the weaker side.
            ...
            // the GsbSlabProgress DTO so every page reads one implementation. The
            // badges show the LAST cut-off's decision and are hidden until one
            // exists — the sides are only decided at 23:59, never intraday.
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <div class="flex items-center justify-between mb-1">
                ...
                <x-help-tip text="Business Volume generated by your Left Genos group today, plus any BV carried over on your Left side from earlier days. This is the figure tonight's 23:59 cut-off will use. Your own purchase BV is not included — the cut-off adds it to whichever side is weaker, and only if a side has reached the first slab. The Power/Weaker badge is the last cut-off's decision.{{ $groupBvTipSuffix }}" />
            </div>
            ...
            @if ($genosBvEligible && $slabProgress)
                ...
                @endif
                <p class="text-xs text-gray-600 mt-1 flex items-center gap-1">{{ $leftBreakdown }}@if ($pendingPersonalBvTip !== null) <x-help-tip :text="$pendingPersonalBvTip" />@endif</p>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            ...
            @if ($genosBvEligible && $slabProgress)
                ...
                @endif
                <p class="text-xs text-gray-600 mt-1 flex items-center gap-1">{{ $rightBreakdown }}@if ($pendingPersonalBvTip !== null) <x-help-tip :text="$pendingPersonalBvTip" />@endif</p>
                @if ($slab1WeakerCfHint !== null && ! $weakerSideIsLeft)
                    <p class="text-xs text-gray-600 mt-1">{{ $slab1WeakerCfHint }}</p>
                @endif
            @endif
