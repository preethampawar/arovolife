...
<div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-sm text-blue-800 mb-6">
        This dashboard shows a live snapshot of your Genos Income. Group BV updates as your Genos members make purchases throughout the day. The 23:59 daily cut-off locks the BV for that day and calculates your Genos Sales Bonus. Your wallet is credited after the cut-off; weekly bonuses transfer to your bank account every Tuesday{{ ($keyDates['hasMonthlyBonuses'] ?? false) ? ', and monthly bonuses in the monthly payout on the 9th' : '' }}. Deductions (3% admin charge, 5% TDS, and any repurchase wallet balance) are applied before transfer.
    </div>
    ...
    <div class="grid grid-cols-1 {{ ($keyDates['hasMonthlyBonuses'] ?? false) ? 'sm:grid-cols-3' : 'sm:grid-cols-2' }} gap-4 mb-6">
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <div class="flex items-center justify-between mb-1">
                ...
                <x-help-tip text="Every day at 23:59 IST your Left and Right group BV is locked and the lower (weaker) side is matched against the Genos Sales Bonus slabs. See the slab ladder on the Genos BV page for exactly where you stand." />
            </div>
            ...
            @else
                <p class="text-xs text-gray-400 mt-1">Locks today's group BV</p>
            @endif
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <div class="flex items-center justify-between mb-1">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        @php
            ...
            $bvBreakdown = static fn (int $todayBv, int $carriedBv, int $topupBv): string =>
                ...
                .($topupBv > 0 ? ' + '.\App\Modules\Shared\Support\IndianNumber::format($topupBv, 0).' personal BV' : '');
