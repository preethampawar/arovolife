...
<div>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-sm text-blue-800 mb-6">
        ...
        @else
            Business Volume generated by your Left and Right Genos groups, day by day. Every figure is a record of purchases that have already happened in your Genos.
        @endif
    </div>
    @enddeveloper
            ...
            <table class="w-full text-sm min-w-[700px]">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-center px-4 py-3 font-semibold text-gray-600">
                            <span class="flex items-center justify-center gap-1">Weaker side <x-help-tip text="Which of your two groups came out lower at this day's 23:59 cut-off, and the BV it was matched on. The cut-off compares each group after carry-forward, so this can differ from the two day figures to the left. The Genos Sales Bonus slab is matched against the weaker side — the stronger (power) side carries forward." /></span>
                        </th>
                        <th class="text-center px-4 py-3 font-semibold text-gray-600">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        @php
                            // The side labels come from what the 23:59 cut-off
                            // actually stored, never from the two raw leg
                            // figures beside them: the cut-off compares each
                            // side AFTER carry-forward, so a day whose Left
                            // leg reads 0 can still be the power side. The
                            // weaker side is the one the power side is not.
                            $powerLabel = match ($row->power_side_after) {
                                'L' => 'Left',
                                'R' => 'Right',
                                default => null,
                            };
