...
it('counts exactly zero when the only BV day of the month is forfeited', function (): void {
    // A group_bv_daily row can never be negative: the columns are unsigned, and
    // a cancelled order's reversal is absorbed down to zero with the remainder
    // carried as a group_bv_debts row (GroupBvReversalService). A forfeited
    // range therefore never exceeds the month it sits in, so the clamp in
    // countedGenosBvForMonth() can only ever meet equality — pinned here at the
    // boundary, where every rupee of the month is forfeited.
    Feature::for(null)->activate(RepurchaseEngineFeature::class);

    $dist = Distributor::factory()->create();
    seedGroupBv($dist->id, '2026-08-25', 5_000_000, 5_000_000); // the month's only BV, forfeited
    seedRankCycle($dist->id, '2026-07-24', '2026-08-23', '2026-08-27');
...
it('subtracts two forfeited ranges in the same month as one total', function (): void {
    // Two failed cycles inside August. Both ranges must come off: dropping
    // either would count 11L or 18L instead of the one compliant day's 3L.
    Feature::for(null)->activate(RepurchaseEngineFeature::class);
    ...
    seedGroupBv($dist->id, '2026-08-22', 8_000_000, 8_000_000);    // forfeited range 2
    ...
    // Month = 26,000,000 paise; the two forfeited ranges total 23,000,000;
    // counted = the 3,000,000 credited on the one compliant day.
