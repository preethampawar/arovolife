...
> residual levels 7–8: ONE shared value over their combined points; income = minC + points × value
> flat level 9: income = minC                              ← no manual override anywhere
> ```
>
> Per-level depth points, payout modes and caps are admin-editable (`fortune_bonus_levels.points_per_member` / `payout_mode` / `cap_paise`); sparse months keep the ABSOLUTE-level treatment (user decision 2026-08-09). Matrix placement (3-wide, 9 levels, monthly reset, FCFS by first GSB credit date) is unchanged. Pool economics — including one `fortune_monthly_pool_levels` row per occupied level — are frozen per month in `fortune_monthly_pools` before any credit (`fortune.pool.frozen` audit row) and never recomputed; re-runs reconstruct incomes from the snapshot. `FortuneDistributionCalculator::allocate()` is the pure allocator, regression-locked to KP's ₹36cr worked example (per-level values ₹13/13/14/15/16/18/19, shared ₹20 residual, ₹3,78,870 leftover).
>
