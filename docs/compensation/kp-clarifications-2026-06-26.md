...
11. **"1+2 rule" → Rank 1 only**, made **admin-configurable** per rank via `rank_tiers.carry_forward_months` (seeded R1=2, rest 0; editable on `/admin/compensation/plan-settings`). ✅ **DONE** (migration + SSOT accessor + `RankQualificationService` + config-driven test). ⚠️ **SUPERSEDED (KP 2026-08-05)** — the 1+2 rule is retired in favour of AO-GO; the `rank_tiers.carry_forward_months` column, its accessor, the rank-form field and the carry-forward creation path were removed on 2026-08-07. Historical `rank_qualifications.is_carry_forward` rows are retained and still read.
